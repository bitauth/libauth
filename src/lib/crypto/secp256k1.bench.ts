...
test('bench: secp256k1: verify signature Low-S, uncompressed pubkey', async (t) => {
  ...
  await suite(t.title, (s) => {
    ...
    const nextCycle = () => {
      ...
      pubkeyUncompressed = secp256k1.derivePublicKeyUncompressed(
        privKey
      ) as Uint8Array;
      ...
      sigDER = secp256k1.signMessageHashDER(privKey, messageHash) as Uint8Array;
      result = false;
    };
    nextCycle();
    s.bench('libauth', () => {
...
test('bench: secp256k1: verify signature Low-S, compressed pubkey', async (t) => {
  ...
  await suite(t.title, (s) => {
    ...
    const nextCycle = () => {
      const privKey = generatePrivateKey(secureRandom);
      messageHash = randomBytes(privateKeyLength);
      pubkeyCompressed = secp256k1.derivePublicKeyCompressed(
        privKey
      ) as Uint8Array;
      ellipticPublicKey = ellipticEc.keyFromPublic(
...
test('bench: secp256k1: derive compressed pubkey', async (t) => {
  ...
  await suite(t.title, (s) => {
    ...
    const nextCycle = () => {
      ...
      pubkeyCompressedExpected = secp256k1.derivePublicKeyCompressed(
        privKey
      ) as Uint8Array;
    };
    nextCycle();
    s.bench('libauth', () => {
      pubkeyCompressedBenchmark = secp256k1.derivePublicKeyCompressed(
        privKey
      ) as Uint8Array;
    });
    s.bench('elliptic', () => {
...
test('bench: secp256k1: create DER Low-S signature', async (t) => {
  ...
  await suite(t.title, (s) => {
    ...
    const nextCycle = () => {
      privKey = generatePrivateKey(secureRandom);
      messageHash = randomBytes(privateKeyLength);
      sigDERExpected = secp256k1.signMessageHashDER(
        privKey,
        messageHash
      ) as Uint8Array;
    };
    nextCycle();
    s.bench('libauth', () => {
      sigDERBenchmark = secp256k1.signMessageHashDER(
        privKey,
        messageHash
      ) as Uint8Array;
    });
...
test('bench: secp256k1: verify: Schnorr vs. ECDSA', async (t) => {
  ...
  await suite(t.title, (s) => {
    ...
    const nextCycle = () => {
      ...
      sigDER = secp256k1.signMessageHashDER(privKey, messageHash) as Uint8Array;
      sigSchnorr = secp256k1.signMessageHashSchnorr(
        privKey,
        messageHash
      ) as Uint8Array;
      result = false;
