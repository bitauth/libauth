...
import { privkey } from './compiler.e2e.spec.helper';
  ...
  if (resultLock.success) {
    ...
    t.deepEqual(resultUnlock, {
      ...
      errors: [
        {
          error:
            'Could not construct the signature "a.schnorr_signature.all_outputs", signing serialization data was not provided in the compilation data.',
          range: {
            endColumn: 33,
            endLineNumber: 1,
            startColumn: 2,
