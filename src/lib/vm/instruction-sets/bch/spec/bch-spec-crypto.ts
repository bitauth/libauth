...
import type { AuthenticationProgramStateBchSpec } from './bch-spec-types.js';
import { ConsensusBchSpec } from './bch-spec-types.js';
...
export const opCheckSigChipLimits =
  <State extends AuthenticationProgramStateBchSpec>(
    {
      secp256k1,
      sha256,
    }: {
  ...
  (s: State) =>
    ...
    useTwoStackItems(s, (state, [bitcoinEncodedSignature, publicKey]) => {
      ...
      if (
        requiredTotalIterations > ConsensusBchSpec.maximumHashDigestIterations
      ) {
        return applyError(
          state,
          AuthenticationErrorBchSpec.excessiveHashing,
            ...
            (finalState, [protocolBugValue]) => {
              ...
              while (
                ...
                if (
                  ...
                  ConsensusBchSpec.maximumHashDigestIterations
                ) {
                  return applyError(
                    state,
                    AuthenticationErrorBchSpec.excessiveHashing,
