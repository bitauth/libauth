...
import { bigIntToBinUint64LE, binToHex, hexToBin, range } from '../../format/format.js';
import type { VmbTestDefinitionGroup } from '../../lib.js';
import { hashTransactionUiOrder } from '../../message/message.js';
  ...
  [
    ...
    [
      ...
      [
        ...
        'OP_UTXOVALUE OP_EQUAL',
        'OP_UTXOVALUE (maximum 8-byte number satoshis)',
        ['invalid'],
      ...
      [
        ...
        'OP_INPUTBYTECODE (ignore result, not 2023 excessive size)',
      ],
      [
        `<1> <0x${range(518)
          .map((i) => binToHex(Uint8Array.of(i)))
          .join('')}>`,
        `OP_DROP <1> OP_INPUTBYTECODE OP_DROP`,
        'OP_INPUTBYTECODE (ignore result, 2023 excessive size)',
        ['2023_invalid'],
      ],
      ...
      ['<2_100_000_000_000_000> <0>', 'OP_OUTPUTVALUE OP_EQUAL', 'OP_OUTPUTVALUE (21,000,000 BCH)', [], { sourceOutputs: [{ lockingBytecode: ['slot'], valueSatoshis: 2_100_000_000_000_000 }], transaction: { inputs: [{ unlockingBytecode: ['slot'] }], outputs: [{ valueSatoshis: binToHex(bigIntToBinUint64LE(21_000_000n * 100_000_000n)) }] } }],
      ['<9223372036854775807> <0>', 'OP_OUTPUTVALUE OP_EQUAL', 'OP_OUTPUTVALUE (maximum 8-byte number satoshis)', ['invalid'], { sourceOutputs: [simpleP2pkhOutput, { lockingBytecode: ['slot'], valueSatoshis: 'ffffffffffffff7f' }], transaction: { outputs: [{ valueSatoshis: binToHex(bigIntToBinUint64LE(9223372036854775807n)) }] } }],
      ...
      [
        ...
        'OP_OUTPUTBYTECODE (ignore result, not 2023 excessive size)',
        ['nonstandard'],
        {
          transaction: {
            outputs: [
      ...
      [
        `<1>`,
        `<0> OP_OUTPUTBYTECODE OP_DROP`,
        'OP_OUTPUTBYTECODE (ignore result, 2023 excessive size)',
        ['nonstandard', '2023_invalid'],
        {
          transaction: {
            outputs: [
